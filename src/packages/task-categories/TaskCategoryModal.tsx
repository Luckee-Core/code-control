...
import { createTaskCategoryThunk, updateTaskCategoryThunk } from '@/store/thunks/task-categories';
...
export const TaskCategoryModal = () => {
  ...
  const isOpen = useAppSelector((state) => state.taskCategoryBuilder.isModalOpen);
  const isLoading = useAppSelector((state) => state.taskCategoryBuilder.isLoading);
  const editingCategoryId = useAppSelector((state) => state.taskCategoryBuilder.editingCategoryId);
  const currentCategory = useAppSelector((state) => state.currentTaskCategory) as TaskCategory | null;
      ...
      <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
        <div className={styles.header}>
          <h2 className={styles.title}>
            {editingCategoryId ? 'Edit Task Category' : 'Create Task Category'}
          </h2>
